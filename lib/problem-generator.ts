...
export class ProblemGenerator {
  ...
  private static generateAdditionProblem(difficulty: DifficultyLevel): Problem {
    ...
    const strategies = ['bridging-to-tens', 'doubles', 'breaking-apart', 'left-to-right']
    ...
    switch (randomStrategy) {
      case 'bridging-to-tens':
        ...
        operand2 = this.getRandomInt(
          Math.max(range.min, (Math.floor(operand1 / 10) + 1) * 10 - operand1 + 5),
  ...
  private static generateSubtractionProblem(difficulty: DifficultyLevel): Problem {
    const range = this.getRange('subtraction', difficulty)
    let operand1: number, operand2: number, strategy: string

    const strategies = ['bridging-down', 'adding-up', 'compensation', 'simple-subtraction']
    const randomStrategy = strategies[Math.floor(Math.random() * strategies.length)]
    ...
    switch (randomStrategy) {
      ...
      case 'compensation':
        ...
        operand2 = Math.floor(this.getRandomInt(range.min + 5, operand1 - 5) / 10) * 10 + lastDigit
  ...
  private static generateMultiplicationProblem(difficulty: DifficultyLevel): Problem {
    const range = this.getRange('multiplication', difficulty)
    let operand1: number, operand2: number, strategy: string

    const strategies = ['doubling', 'times-5', 'times-9', 'times-11', 'simple-tables']
    const randomStrategy = strategies[Math.floor(Math.random() * strategies.length)]
    ...
    switch (randomStrategy) {
      ...
      case 'times-5':
        operand1 = this.getRandomInt(range.min * 2, Math.min(range.max * 4, 99))
        operand2 = 5
        strategy = 'times-5'
        break

      case 'times-9':
        operand1 = this.getRandomInt(range.min + 2, Math.min(range.max + 10, 50))
        operand2 = 9
        strategy = 'times-9'
        break
  ...
  private static generateDivisionProblem(difficulty: DifficultyLevel): Problem {
    ...
    const strategies = ['factor-recognition', 'multiplication-inverse', 'simple-division']
    const randomStrategy = strategies[Math.floor(Math.random() * strategies.length)]
    ...
    switch (randomStrategy) {
      ...
      case 'multiplication-inverse':
        ...
        operand2 = this.getRandomInt(range.min + 2, Math.min(range.max / 5, 12))
        quotient = this.getRandomInt(range.min + 3, Math.min(range.max / operand2, 15))
        operand1 = operand2 * quotient
      ...
      default:
        // Simple division
        operand2 = this.getRandomInt(range.min + 1, Math.min(range.max / 3, 10))
        quotient = this.getRandomInt(range.min + 1, Math.min(range.max / operand2, 12))
        operand1 = operand2 * quotient
  ...
  static generateSession(
    ...
    if (preferences.enabledOperations.addition) enabledOperations.push('addition')
    if (preferences.enabledOperations.subtraction) enabledOperations.push('subtraction')
    if (preferences.enabledOperations.multiplication) enabledOperations.push('multiplication')
    if (preferences.enabledOperations.division) enabledOperations.push('division')
    ...
    for (let i = 0; i < sessionLength; i++) {
      ...
      const randomOperation = enabledOperations[
        Math.floor(Math.random() * enabledOperations.length)
      ]

      const problem = this.generateProblem(randomOperation, preferences.difficultyLevel)
    ...
    switch (strategyCategory) {
      case 'bridging-to-tens':
        return {
          ...
          description: 'Make one number a round ten to simplify the calculation',
          steps: [
